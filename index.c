...
int index_load(Index *index) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        unsigned int mode;
        if (sscanf(line, "%o %s %lu %u %s", &mode, hex,
                   &entry->mtime_sec, &entry->size, entry->path) != 5) {
            continue;
        }
...
int index_save(const Index *index) {
    ...
    for (int i = 0; i < index->count; i++) {
        ...
        fprintf(fp, "%o %s %lu %u %s\n", entry->mode, hex,
                (unsigned long)entry->mtime_sec, entry->size, entry->path);
    }
...
int index_add(Index *index, const char *path) {
    ...
    ObjectID id;
    int rc = object_write(OBJ_BLOB, data, size, &id);
    free(data);
    if (rc != 0) {
